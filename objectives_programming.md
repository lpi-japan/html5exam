...
- [1. Web プロフェッショナルプログラミング Level 1 試験 出題範囲](#1-web-プロフェッショナルプログラミング-level-1-試験-出題範囲)
  ...
  - [1.9. セキュリティ](#19-セキュリティ)
    - [1.9.1. 実行コンテキストとオリジン (重要度: 2)](#191-実行コンテキストとオリジン-重要度-2)
    - [1.9.2. API 権限とデバイスアクセス (重要度: 2) / 旧 1.5.2](#192-api-権限とデバイスアクセス-重要度-2--旧-152)
    - [1.9.3. 依存の取り込み (重要度: 2)](#193-依存の取り込み-重要度-2)
...
- セキュアコンテキストやクロスオリジン制約が通信に与える影響は 1.9 節と合わせて理解する。
...
- 依存の取り込みが及ぼす影響（セキュリティ・互換性の観点）。詳細は 1.9.3 節も参照。

## 1.9. セキュリティ

### 1.9.1. 実行コンテキストとオリジン (重要度: 2)
出題種別: 知識問題、記述問題
...
### 1.9.2. API 権限とデバイスアクセス (重要度: 2) / 旧 1.5.2
出題種別: 知識問題、記述問題
