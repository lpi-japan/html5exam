...
- Level1
  - JavaScript の基本文法を理解し、簡単なスクリプトの動きを把握できる。
  - オブジェクトやクラスが有効な場面を理解し、適切に活用してコーディングできる。
  - 非同期処理の必要性やメリット・制約を理解し、エラーハンドリングを含めて実装できる。
  - ブラウザ開発者ツールなどを用いてコードの挙動確認や問題箇所の調査ができる。
  - DOM 操作や基本的なイベント処理（ユーザー操作、フォーム操作、ページロードなど）を実装できる。
  - 主要な Web API について、それらの目的や効果範囲を理解している。
  - 通信や Web API 利用に関わるセキュリティ設定の目的と影響を理解している。 
- Level2
