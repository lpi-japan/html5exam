...
- Level1
  - JavaScriptの基本文法を理解し、シンプルなスクリプトを記述できる
  - DOM操作や基本的なイベント処理（クリックやフォーム操作など）の実装が可能
  - 例外処理とデバッグ手法を理解し、エラー発生時にトラブルシューティングができる
  - HTML/CSSとの連携により、基本的なWebページを構築できる

- Level2
  - 非同期処理やエラーハンドリングの実践的理解と利用ができる
  - モジュール化、パッケージ管理を利用し、大規模なコードベースで設計が可能
  - 高度なDOM操作、カスタムイベント、パフォーマンス最適化手法を活用した実装経験がある
  - セキュリティやアクセシビリティ、テスト自動化などの高度なWeb開発手法を実践できる
...
- [2. JavaScript Level 2 出題範囲](#2-javascript-level-2-出題範囲)
  ...
  - [2.5. グラフィックス・アニメーション / 1.5.1](#25-グラフィックスアニメーション--151-1)
    - [2.5.1. Canvas(2D) / 2.3.1](#251-canvas2d--231-1)
    - [2.5.2. SVG / 2.3.2](#252-svg--232-1)
