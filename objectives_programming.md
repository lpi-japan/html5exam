...
スクリプトの実行と Web API の利用について、挙動と前提を踏まえた判断ができる。

- **スクリプトの実行** — JavaScript の文法・型・制御構文を理解し、スクリプトの実行結果を予測できる。
- **非同期** — 非同期処理の仕組みと制約を理解し、待ち状態やエラーが生じる場面を見通せる。
- **インタラクション** — スクリプトとイベントにより、ユーザー操作や DOM の状態変化に動的に応答できる。
...
- [2. Web プロフェッショナルプログラミング Level 2 試験 出題範囲](#2-web-プロフェッショナルプログラミング-level-2-試験-出題範囲)
  ...
  - [2.3. グラフィックス・アニメーション / 旧 1.5.1](#23-グラフィックスアニメーション--旧-151)
    - [2.3.1. Canvas(2D) (重要度: 3) / 旧 2.3.1](#231-canvas2d-重要度-3--旧-231)
