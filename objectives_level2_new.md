...
#### 詳細
- CSSカスタムプロパティ（`--*`）を利用して、テーマ（例: ライト/ダークモード）やレイアウトの柔軟な調整が可能を理解している。
...
#### 詳細
- WebSocketを使った通信の利点と欠点を理解している。
- WebSocketにおけるイベント発生タイミングを理解している。
- WebSocketを使ったサーバとの通信（クライアント側のコード）を理解している。
...
#### 詳細
- 各技術の通信方向・接続形態の違い（双方向・サーバプッシュ・ピアツーピア）を理解している。
- WebRTC の概要（ブラウザ間のリアルタイム通信、シグナリングの必要性）を理解している。
- Push API の概要（Service Worker と連携したプッシュ通知）を理解している。
...
#### 詳細
- Indexed Database APIの特徴と、Web Storageとの違いを理解している。
  - データベースへのアクセス
  - データの読み込み
  - データの保存
...
#### 詳細
- HTML5におけるローカルファイルアクセス機能の概要を理解している。
- ローカルファイルアクセスにおけるセキュリティ観点での制限事項を理解している。
- ファイルオブジェクトリストを取得できる。
- ローカルファイルの読み込みを理解している。
...
#### 詳細
- Geolocation APIの特徴と注意する点を理解している。
- 端末における現在の位置情報を取得する方法を理解している。
- 現在位置取得後のコールバック関数呼び出しを理解している。
