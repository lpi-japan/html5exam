...
| 編成 | アクセシビリティはマークアップ Level 1/2 でどう分ける？ | Markup | 現状ARIAの一部だけLevel1にしているが、ARIAはすべてLevel2というのもあり得るか？ → 現行分担（mk L1 はランドマーク・名前付け等の静的な基本 ARIA、L2 は widget role・動的 ARIA・フォーカス制御）を維持で確定。基本 ARIA は汎用の基本作業として境界基準（改訂概要 3.3）に照らして L1 側で整合するため。 | | 解決 |
...
| 項目追加 | キーワード表 L2 環境差行の「ウィンドウオブジェクトの寸法・スクロール」に対応する内容が L2 本文にない。どこに置くか？ | Programming | L2 8.1（DOM応用）に配置。ウィンドウ・要素の寸法とスクロール位置の取得（`getBoundingClientRect()`・`scrollIntoView()`）、`resize`/`scroll` イベントと Observer 系 API の使い分けを追記。基本操作は pg L1 4.4 が担う階層分担をメモで明示。 | | 解決 |
| 項目追加 | キーワード表 L2 メディア行の `getUserMedia`・MediaStream・EME に対応する内容が L2 本文にない。追加するか？ | Programming | `getUserMedia`/MediaStream は境界基準（改訂概要 3.3）に沿って再整理: pg L1 7.2 は API 横断の権限モデルに限定し、個別 API の利用（constraints 指定・`srcObject` 接続・トラック操作）は L2 メディア主題に 5.7 節「カメラ・マイクと MediaStream」として新設（重要度は仮置き2）。これでキーワード表 L2 メディア行との対応も取れた。EME（Encrypted Media Extensions）は DRM の運用が前提で出題に適さないため範囲外と確定し、L2 5.7 のメモに明示（キーワード表に対する意図的な非対応として扱う）。 | | 解決 |
| 項目追加 | キーワード表 L2 デバッグ行 ○「セキュリティ・パフォーマンス調査でのデバッグツール活用」に対応する記載がセキュリティ側（12.1）にしかない。パフォーマンス側にも置くか？ | Programming | L2 13.2 に「開発者ツールのパフォーマンス・ネットワークパネルによるボトルネック調査」を追記し、デバッグ基礎（pg L1 6.3）との階層分担をメモで明示。 | | 解決 |
...
| 方針 | L1 のみ先行リリースする可能性があるため、L1/L2 の境界の根拠を説明できる形で明文化する必要がある。どう言語化するか？ | Both | 改訂概要に 3.3 節「レベル1とレベル2の境界の考え方」を新設。レベル間の違いを領域差ではなく確認する能力の違いとして定義したうえで、過去の個別判断（transform 移動・基本トランジション残置・カスケードキーワード L2 配置・フォーム具体化・マルチカラム移動等）から帰納した3基準（問いの型／登場範囲／前提知識の量）を置いた。境界付近の項目への適用結果は概要の肥大化を避けるため本ファイル末尾の「2. L1/L2 境界付近の項目の判定」に置く。 | | 解決 |
...
| 内容増減 | L2「用途別Web API」主題が旧 API カタログの寄せ集めのままで厚すぎる。改訂概要のキーワード表は動かさず、本文だけで圧縮できるか？ | Programming | 8節→4節に統合し掘り下げを浅くした: (1) WebSocket・SSE・技術選択・WebRTC/Push概要 →「リアルタイム通信」、(2) IndexedDB・File・バイナリ →「クライアントストレージとファイル」、(3) Service Worker（キャッシュの基本に圧縮）、(4) Geolocation（重要度2→1）。試験全体に占める用途別 API の割合を減らす方針に合わせた。 | | 解決 |
| 章立て | L2 メディア主題が旧 L2 の節の寄せ集めで最も厚い（7節）。本文だけで整理できるか？ | Both | グラフィックス系3節（Canvas(2D)・SVG・Animation frames）を「グラフィックスとアニメーションフレーム」1節（重要度3）に統合し、Canvas/SVG 比較を1か所に集約。5.3 メディア要素の API はカタログ列挙（再生・停止・状態取得が並ぶだけ）だった一段を3項目に要約。7節→5節、主題行数は約2割減。ストリーミング（5.1）・可変フォント（5.2）はキーワード表メディア行の記載に対応するため主題内に維持。 | | 解決 |
| 項目修正 | L2 で重要度が高いのに節の中身が薄く、ユニークな問題を揃えられない組み合わせはないか？ | Both | 重要度は重要さと出題数を兼ねる前提で是正: 2.1 アクセシビリティ実践 6→3、2.2 モーダル 4→2、1.1 構造化データ 5→3、3.2 Flex/Grid応用 6→3、4.1 カスタムプロパティ 4→2（併せて文体崩れ「調整が可能を理解している」を修正）、13.1 CSSパフォーマンス 4→3、3.1 論理プロパティ・段組 5→4（5 に見合う出題を揃えられないため）。なお新設の設計判断系（9.1・10.1・11.1）の重要度 3 は据え置き: 看板ではあるが、受験者から見て新規差分が重要度の大半を占めるとストレスが大きいため、高すぎない方がよいという判断。 | | 解決 |
