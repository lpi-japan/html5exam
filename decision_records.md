...
| 項目追加 | キーワード表 L2 メディア行の `getUserMedia`・MediaStream・EME に対応する内容が L2 本文にない。追加するか？ | Programming | (未判断) `getUserMedia`/MediaStream の基礎はプログラミング L1 の 7.2 節（API 権限とデバイスアクセス）にあり、L2 で応用（録画・WebRTC 連携等）まで掘るかは未決。EME（Encrypted Media Extensions）は DRM 前提で出題しにくく、扱いを含め要検討。 | | |
