...
| 章立て | Level 2 の主題をどう再編するか？（マークアップ系・プログラミング系の verbatim 統合状態からの整理） | Both | キーワード表（改訂概要 3.2 節）の L2 列に整合する 14 主題へ再編: セマンティクスと文書設計／アクセシビリティと対話的UI／高度なレイアウトと視覚効果／再利用・構成／メディア／JavaScriptの高度な言語機能／モジュールと開発基盤の応用／DOM応用と履歴管理／状態管理とサーバ連携／バリデーション／認証・認可／セキュリティ／パフォーマンス／用途別Web API。◎ 行を主題タイトルとし、○ 行（視覚表現・操作への応答・環境差・プログラムの基本構造など）の高度な内容は関連主題の節として配置。二重だった `# 2.` 章番号は単一の連番に統一。重要度スケールの正規化（マークアップ系1〜6とプログラミング系1〜3が混在）は未実施で、今後の課題。 | | 解決 |
