...
| 章立て | mk L1 の主題を6本に再編するか？キーワードと現行名の中間の主題名とする | Markup | 6主題: 通信の基礎 / HTML文書と意味 / CSSの基礎 / レイアウトと視覚表現 / 操作とフォーム / 表示環境とメディア（主題1・2は後続で再編）。セキュリティ○・Webフォント・iframe は 6.5、Flex/Grid は 6.3。 | | 解決 |
| 章立て | mk L1 主題1を通信に専念させるか？ | Markup | 主題1を「通信の基礎」に改称。副題: 1.1 クライアント・サーバとHTTP/HTTPS、1.2 URLとWebリソース、1.3 ヘッダの概要、1.4 Cookieとセッション（いずれも旧1.1.1の再配置）。HTML書式は主題2.1、Unicodeは2.3へ移設。フォーム method/action/enctype は5.1。方針は改訂概要のスキル記述に整合。 | ver.2.5はHTTP先 | 解決 |
