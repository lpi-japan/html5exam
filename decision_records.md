...
| Fetch API | 既定動作の理解・切り分け pg L1／要件に応じた制御 L2 | 一つのリクエストの成立とブラウザ既定（キャッシュされうること等）の切り分けは L1。認証情報付与・中断・キャッシュモード選択・ストリーミング逐次消費は要件に応じて既定を変える制御として L2（旧 XHR 節の逐語置換に由来した「詳細さ」分割を、能力差に書き換えた。2026-07） |
