...
| 方針 | L1 のみ先行リリースする可能性があるため、L1/L2 の境界の根拠を説明できる形で明文化する必要がある。どう言語化するか？ | Both | 改訂概要に 3.3 節「レベル1とレベル2の境界の考え方」を新設。過去の個別判断（transform 移動・基本トランジション残置・カスケードキーワード L2 配置・フォーム具体化・マルチカラム移動等）から帰納した3基準（問いの型／登場範囲／標準の安定度）＋構造規則（mk L1 は JS 非前提・pg L1 はマークアップ詳細非前提・L2 は両 L1 前提）として整理し、境界事例11件の判定表を付した。基準1・2が表すレベル間の能力差の定義は改訂方針（2 節）にも箇条書き1項目として明記し、3.3 はそれを項目単位の判定に落とした基準と位置づけた。 | | 解決 |
