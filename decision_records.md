...
| 内容増減 | CSS全域キーワード（`inherit`, `initial`, `unset`, `revert`）と `all` プロパティは新試験でどのレベルに追加するか？ | Markup | 動作を知るだけでなく「どのシーンでどのキーワードを選ぶか」の判断力が問われるため Level 2 向きと判断。カスタムプロパティとは別概念なので 2.3.1 には入れず、新たに 2.3.5「カスケードキーワードの使い分け」節を設けて配置。現行 v2.5 には記載なし（新規追加トピック）。 | | 解決 |
