...
| 項目追加 | CSS利用統計で上位に来る cursor, outline は入れる？ | Markup | Level 1 の「CSSによる操作と状態」（旧検討時の仮称 1.4.2）に追加。level1 でよいかは未決。 | | |
...
| 章立て | mk L1 の主題を6本に再編するか？キーワードと現行名の中間の主題名とする | Markup | v2.5 Level 1 の主題構成（Webの基礎／要素／CSSの基礎／CSSデザイン／レスポンシブ 等）を、キーワード軸の6主題（通信の基礎／HTML文書と意味／スタイルと視覚表現／操作とフォーム／表示環境／メディア）へ再編。iframe・混在コンテンツはメディア節、Flex/Grid は表示環境節へ。 | | 解決 |
| 章立て | mk L1 主題1を通信に専念させるか？ | Markup | v2.5 では旧 1.1.1 に HTTP と HTML 書式等が混在。新 Level 1 では主題1を「通信の基礎」とし、クライアント・サーバとHTTP/HTTPS・URL・ヘッダ・Cookie/セッションに分割。HTML書式・Unicodeは主題2へ、フォームの method/action/enctype は操作とフォーム節へ移す（v2.5 も HTTP を先に置いていた流れの延長）。 | | 解決 |
| 章立て | mk L1 主題3・4を統合し主題6を分割するか？ | Markup | v2.5／改訂途中案で分かれていた CSS 基礎とレイアウト・視覚表現を、スキル「プレゼンテーション」に対応する「スタイルと視覚表現」へ統合。レスポンシブ周辺とメディアを、スキル「環境差」「メディア」に対応する「表示環境」「メディア」へ分割（内容の新規追加は伴わない再配置）。 | | 解決 |
...
| 内容増減 | v2.5 にない CSS の操作・状態系プロパティ（`cursor`, `pointer-events`, `user-select`, `outline`）および基本トランジションを Level 1 に置くか？ | Markup | `cursor` と `outline` はフォーカス可視化・操作感の基礎として実務頻度が高く Level 1 に追加が妥当。`pointer-events`, `user-select`、擬似クラス、基本的な `transition-*` も同節にまとめる。節名はスキル語の「インタラクション」では広すぎるため「CSSによる操作と状態」とする（キーフレームアニメーションは変形とアニメーション節に残す）。 | | |
