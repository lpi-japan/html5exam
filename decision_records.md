...
| 項目修正 | L2 で重要度が高いのに節の中身が薄く、ユニークな問題を揃えられない組み合わせはないか？ | Both | 重要度は重要さと出題数を兼ねる前提で是正: 2.1 アクセシビリティ実践 6→3、2.2 モーダル 4→2、1.1 構造化データ 5→3、3.2 Flex/Grid応用 6→3、4.1 カスタムプロパティ 4→2（併せて文体崩れ「調整が可能を理解している」を修正）。スケール全体の正規化（mk 1〜6 / pg 1〜3 混在）は別課題として残す。 | | 解決 |
