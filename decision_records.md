...
| --- | --- | -------- | ------------------------------------------- | --------------------- | -------- |
| 編成 | 認定シリーズ名 | Both | Webプロフェッショナル認定？ | https://html5exam.jp/outline/ |  |
| 編成 | 認定名 | Both | Webプロフェッショナルマークアップ、Webプロフェッショナルプログラミング？ | |  |
| 編成 | 階層名 | Both | これまでどおりレベル1，2でよいか | |  |
...
| 編成 | svgやcanvasは現行Level2だったのでprogramming側では言及予定だが、マークアップ側で何も言及しないのは違和感がある。どうするか？ |  |  |  |  |
...
| 新トピック | クライアント・サーバー間通信の実装 (REST, GraphQL, gRPC, tRPC) を出題するか？もしする場合、どんな趣旨の問題になるか？ | Programming | 混ぜ込み済み | | 解決 |
...
| 新トピック | emojiは？ | Markup | マークアップレベル1へとりあえず追加 | | 解決 |
