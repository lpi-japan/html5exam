...
- **セマンティクス** — HTML の要素・属性が担う意味と構造を理解し、コンテンツの内容と目的に応じたマークアップを選び分けられる。
- **プレゼンテーション** — CSS のセレクタ・カスケード・プロパティの規則を理解し、スタイル指定が最終的な見た目にどう反映されるか（適用の優先・継承・プロパティ値の効果）を読み取れる。
- **インタラクション** — HTML 要素や CSS により実現するインタラクション（フォーム、状態を持つ要素、擬似クラス、トランジション等）の意味と制約を理解し、利用場面を見極められる。
- **環境差** — ビューポート、画面サイズ、メディア条件などの環境差を理解し、同一のマークアップ・スタイルがデバイスごとにどう見え方やレイアウトを変えるかを見極められる。
- **メディア** — 画像・音声・動画や Web フォントの埋め込みと配信最適化を理解し、形式や属性の選択ができる。
...
- 入力制御や送信のプログラム的な操作はプログラミング試験（1.7 節）の範囲とする。
